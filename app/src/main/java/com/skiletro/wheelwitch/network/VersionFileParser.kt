...
/**
 * Parses RetroRewindVersion.txt format: lines of "version url path description". Skips malformed
 * lines.
 */
 ...
 *
 * Leaves anything HttpUrl can't parse unchanged so a malformed entry fails through to the normal
 * skip path in [parseUpdatesText].
 */
...
internal fun parseDeletionsText(text: String): List<DeletionEntry> {
  return text
      ...
      .mapNotNull { line ->
        ...
        DeletionEntry(version, parts[1].trim().removePrefix("/"))
      }
}
...
object VersionFileParser {
  ...
  private val probeClient
    get() = HttpClientProvider.probeClient
  ...
  /**
   * Fetches the full server health report. Returns a basic ServerHealth if the detailed endpoint
   * fails.
   */
  ...
  /**
   * Quick reachability check using the short-timeout [probeClient]. Returns true if the server is
   * reachable and responds successfully. Used by VMs to short-circuit before expensive timeouts.
   */
  fun probeServer(): Boolean =
      runCatching {
  ...
  /**
   * Fetches the time trial leaderboard for a given track, filtering by engine class and glitch
   * preference. Paginated via [page] and [pageSize].
   */
  fun fetchTrackLeaderboard(
    ...
    val url =
        "$TIME_TRIAL_LEADERBOARD_URL?glitchAllowed=$glitchAllowed&trackId=$trackId&cc=$cc&page=$page&pageSize=$pageSize"
    val json = fetchUrl(url)
  ...
  fun fetchPlayerLeaderboard(friendCode: String): Result<PlayerLeaderboardData> = runCatching {
    ...
    PlayerLeaderboardData(
        ...
        miiData =
            if (rawMiiData == null || rawMiiData === JSONObject.NULL) null
            else rawMiiData.toString(),
    )
  }
  ...
  private fun badgesForPid(pid: Long): List<BadgeType> =
      runCatching {
