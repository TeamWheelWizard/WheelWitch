...
/**
 * Serialises [entries] (friend code -> leaderboard data) into a single JSON blob. Null
 * [PlayerLeaderboardData.name]/[PlayerLeaderboardData.miiData] are omitted and reconstructed as
 * null by [decodeLeaderboardCache], so the "API supplied nothing" case stays distinct from an
 * absent entry.
 */
...
/**
 * Parses the JSON blob produced by [encodeLeaderboardCache]; blank or malformed input yields an
 * empty map.
 */
        ...
        while (keys.hasNext()) {
          ...
          result[friendCode] =
              PlayerLeaderboardData(
                  ...
                  name =
                      if (rawName == null || rawName === JSONObject.NULL) null
