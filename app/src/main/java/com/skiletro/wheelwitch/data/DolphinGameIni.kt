...
/**
 * Pure INI string manipulation used by [DolphinTree.ensureRmcGameInis] to force-disable cheats and
 * RetroAchievements in Dolphin's `GameSettings/` and `Config/RetroAchievements.ini` files.
 *
 * All functions are side-effect free and operate on INI text, mirroring the [DolphinConfig] pattern
 * so they can be unit tested without any Android dependencies.
 */
object DolphinGameIni {
  ...
  /**
   * Key of the "Achievements.Achievements" section of INI files which determines whether
  ...
  /**
   * Ensures [key] = [value] exists under [section] in the INI [content] by inserting a line for it
  ...
  fun addIniKeyValue(
    ...
    for (line in lines) {
      outLines.add(line)
      extractIniSection(line)
          ?.takeIf { !added && it == section }
          ?.let {
            outLines.add(kvLine)
            added = true
          }
  ...
  /**
   * Ensures [key] = [value] exists under [section] in the INI [content] by appending lines at the
   * end for it.
   */
  ...
  /**
   * Returns the section name including the brackets if [line] declares an INI section, or null
   * otherwise.
   */
  ...
  /**
   * Renames all occurrences of a section [oldSection] (case-insensitive match) to [newSection] and
   * returns the result.
   */
  fun renameSection(
      content: String,
  ...
  /**
   * Removes every line matching [key] (case-insensitive key name) within [section] in the INI
   * [content]. Returns the modified content, or the original if nothing changed.
   */
  fun removeIniKeyInSection(
    ...
    for (line in lines) {
      extractIniSection(line)?.let { declaredSection ->
        currentSection = declaredSection
      }
          ?: run {
